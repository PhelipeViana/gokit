...
description: Como trabalhar no motor de migrations do GoKit e validar qualquer mudança nos quatro dialetos (Oracle, Postgres, MySQL, SQL Server). Use ao alterar o DSL (migration/, acao/), o parser AST (internal/migrationgo/), o executor (internal/migraterun/) ou o seed; ao investigar erro de migration/seed; ao adicionar operação nova; ou quando aparecer ORA-, SQLSTATE, "Could not create constraint", drift de checksum ou divergência de sequência/identity.
---
...
| Parser AST | [internal/migrationgo/parser.go](internal/migrationgo/parser.go) |
